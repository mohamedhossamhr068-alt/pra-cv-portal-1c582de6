  ...
  .handler(async ({ data, context }) => {
    try {
      ...
      if (!hasKeyword) {
        ...
        if (!allowed) return { inserted: 0, skipped: 0, error: "FORBIDDEN" };
      }
      ...
      if (hasKeyword) {
        const { data: profile } = await supabase
          ...
          .eq("id", userId)
          .maybeSingle();
        if (profile?.is_blocked) return { inserted: 0, skipped: 0, error: "ACCOUNT_BLOCKED" };
        const nextCount = (profile?.job_search_count ?? 0) + 1;
        const shouldCharge = nextCount % 2 === 0; // every 2nd search costs 1 credit
        if (shouldCharge && (profile?.credits ?? 0) < 1) return { inserted: 0, skipped: 0, error: "NO_CREDITS" };
        await supabase
          .from("profiles")
          .update({
      ...
      if (!apiKey) return { inserted: 0, skipped: 0, error: "FIRECRAWL_NOT_CONFIGURED" };
      ...
      if (!kw) {
        ...
        kw = String(out?.targetRole ?? out?.headline ?? out?.experience?.[0]?.role ?? out?.personalInfo?.title ?? "")
          .trim()
          .split("\n")[0]
          .slice(0, 60);
      }
      ...
      for (const { q, source } of queries) {
        try {
          ...
          for (const r of results) {
            ...
            if (
              /\b(saudi|riyadh|jeddah|dubai|abu dhabi|qatar|kuwait|oman|bahrain|usa|united states|uk|london)\b/i.test(
                `${r.title} ${desc}`,
              )
            )
              continue;
            ...
            collected.push({
              ...
              seniority: /senior|lead|principal/i.test(r.title)
                ? "senior"
                : /junior|entry|intern/i.test(r.title)
                  ? "junior"
                  : "mid",
              location: "Egypt",
              country: "EG",
      ...
      if (error) return { inserted: 0, skipped: 0, error: `DB_ERROR: ${error.message}` };

      return { inserted: collected.length, skipped: 0 };
    } catch (err: any) {
      console.error("[scrapeEgyptJobs] Unhandled error:", err?.message ?? err);
      return { inserted: 0, skipped: 0, error: err?.message ?? "UNKNOWN_ERROR" };
    }
  });
